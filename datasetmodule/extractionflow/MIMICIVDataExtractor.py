...
class MimicIVDataExtractor(DataExtractor):
    ...
    def person(self, sub):
        return f"""select distinct P.person_id as {sub}
                   from `masterthesis-401512.mimiciv_cdm_2021_07_30.cdm_condition_occurrence` CO
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.cdm_person` P on P.person_id = CO.person_id
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept` C on C.concept_id = CO.condition_source_concept_id
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.ccs_single` CCS on CCS.icd9 = CO.condition_source_value
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.icd9_filtered` FILT on FILT.level4 = CO.condition_source_value
                   where C.vocabulary_id = 'ICD9CM'
                   and CO.condition_source_value in (
                       select distinct FILT.level4 from `masterthesis-401512.mimiciv_cdm_2021_07_30.icd9_filtered` as FILT
                       join `masterthesis-401512.mimiciv_cdm_2021_07_30.cdm_condition_occurrence` CO on CO.condition_source_value = FILT.level4
                       join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept` C on C.concept_id = CO.condition_source_concept_id
                       where C.vocabulary_id = 'ICD9CM'
    ...
    def diagnosis(self, sub):
        return f"""select distinct FILT.level4 as {sub} 
                   from `masterthesis-401512.mimiciv_cdm_2021_07_30.icd9_filtered` as FILT
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.cdm_condition_occurrence` CO on CO.condition_source_value = FILT.level4
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept` C on C.concept_id = CO.condition_source_concept_id
                   where C.vocabulary_id = 'ICD9CM'
                   group by FILT.level4
    ...
    def medication(self, sub):
        return f"""select distinct C2.concept_code as {sub}
                   from `masterthesis-401512.mimiciv_cdm_2021_07_30.cdm_drug_exposure` DE
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.cdm_person` P on P.person_id = DE.person_id
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept` C1 on C1.concept_id = DE.drug_concept_id
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept_ancestor` CA on CA.descendant_concept_id = C1.concept_id
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept` C2 on C2.concept_id = CA.ancestor_concept_id
    ...
    def labtest(self, sub):
        return f"""select distinct C.concept_code || '-N' as {sub}
                   from `masterthesis-401512.mimiciv_cdm_2021_07_30.cdm_measurement` M
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept` C on M.measurement_concept_id = C.concept_id
    ...
    def procedure(self, sub):
        return f"""SELECT DISTINCT C.concept_code as {sub}
                   FROM `masterthesis-401512.mimiciv_cdm_2021_07_30.cdm_procedure_occurrence` CO
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept` C on C.concept_id = CO.procedure_concept_id
    ...
    def person_labtest(self, sub, obj):
        return f"""select distinct M.person_id as {sub}, CASE when L.flag is NULL then C.concept_code || '-N' else C.concept_code || '-A' end as {obj} 
                   from `masterthesis-401512.mimiciv_cdm_2021_07_30.cdm_measurement` M
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept` C on M.measurement_concept_id = C.concept_id
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.src_labevents` L on M.trace_id = L.trace_id"""
    ...
    def atc_code_features(self):
        return f"""select distinct C2.concept_code as medication, C3.concept_code as atc_code
                   ...
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept_ancestor` CA on CA.descendant_concept_id = C1.concept_id
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept` C2 on C2.concept_id = CA.ancestor_concept_id
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept_relationship` CR on C2.concept_id = CR.concept_id_1
                   join `masterthesis-401512.mimiciv_cdm_2021_07_30.voc_concept` C3 on C3.concept_id = CR.concept_id_2
                   where DE.drug_concept_id > 0
                   and CA.min_levels_of_separation = 1
                   and C2.vocabulary_id = 'RxNorm'
                   and C2.concept_class_id = 'Clinical Drug Form'
